...
`replayglowz_lab` is the ReplayGlowz transcript worker. It is a standalone FastAPI service that accepts transcript jobs from the main ReplayGlowz system, downloads and normalizes audio, runs a transcription provider, and returns normalized transcript data.
