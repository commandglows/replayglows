...
evidence:
  ...
  - ".env.example"
next_step: "Review assumptions against the main ReplayGlows app and production deployment."
---
...
`lab` is the ReplayGlows transcript worker. It is a standalone FastAPI service that accepts transcript jobs from the main ReplayGlows system, downloads and normalizes audio, runs a transcription provider, and returns normalized transcript data.

This repository is not the full ReplayGlows product. Treat it as a narrow infrastructure component with a stable HTTP contract.
...
- Preserve the request and response contract used by the main ReplayGlows app.
