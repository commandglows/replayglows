...
When ReplayGlows asks for a transcript from a provider that needs audio processing, the worker:
...
```bash
cd /home/claude/lab
python3 -m pip install --target /tmp/replayglows-worker-audit --require-hashes -r requirements.lock
pip-audit --path /tmp/replayglows-worker-audit
```
...
```bash
cd /home/claude/lab
docker build -t replayglows-transcript-worker .
```
...
- keep the main ReplayGlows app as its own PM2 environment
- run the transcript worker as a second PM2 environment from `lab`
...
6. Open the ReplayGlows app and confirm local worker-backed providers are shown as available.
...
- `openai_mini`, `openai`, and `deepgram` additionally require a user API key saved in ReplayGlows
...
3. Confirm ReplayGlows first attempts `youtube_captions` when enabled.
...
- `Missing openai API key for transcript provider openai.` or similar
  The user has not saved the provider key in ReplayGlows yet.
...
- `openai_mini` / `openai`
  - require a user API key saved in ReplayGlows
- `deepgram`
  - requires a user API key saved in ReplayGlows

## Important product note

For non-technical users, this worker should generally run in the cloud and be operated by ReplayGlows.
That way:
