...
`lab` is a dedicated transcript worker for ReplayGlows. Its job is to accept a transcription request from the main app, fetch and normalize YouTube audio, run one transcription provider, and return normalized transcript data plus warnings and optional cost estimates.
