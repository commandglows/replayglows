...
| --- | --- | --- | --- | --- |
| `replayglowz_app/lib/**` | Flutter app | `shipflow_data/technical/apps/replayglowz_app/architecture.md` | `(cd replayglowz_app && flutter analyze)` | Auth, routing, Convex client, screens, models, providers, i18n, or widget behavior changes. |
| `replayglowz_app/api/**` | Vercel YouTube OAuth handlers | `shipflow_data/technical/apps/replayglowz_app/architecture.md` | `(cd replayglowz_app && node --test api/auth/_youtube.test.js)` | OAuth request/return flow, cookie handling, token exchange, Clerk, or Convex mutation behavior changes. |
...
| `replayglowz_backend/packages/backend/convex/**` | ReplayGlowz product Convex backend | `shipflow_data/technical/architecture.md` | `(cd replayglowz_backend/packages/backend && npm run typecheck)` | Schema, auth provider, product access, YouTube tokens, settings, videos, playlists, notes, transcripts, or function names change. |
| `replayglowz_site/src/pages/**`, `replayglowz_site/src/components/**`, `replayglowz_site/src/i18n/**` | Astro public site | `shipflow_data/technical/apps/replayglowz_site/architecture.md` | `(cd replayglowz_site && npm run build)` | Public route, CTA, pricing, claim, i18n, layout, or component changes. |
...
| `replayglowz_lab/server.py`, `replayglowz_lab/main.py` | Transcript worker | `shipflow_data/technical/apps/replayglowz_lab/architecture.md` | `(cd replayglowz_lab && python -m py_compile main.py server.py)` | API contract, auth, limits, providers, queueing, media handling, or health behavior changes. |
