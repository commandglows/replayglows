...
supersedes: []
evidence:
  - "app/api/auth/youtube.js"
  - "app/api/auth/youtube/callback.js"
  - "app/api/auth/_youtube.js"
...
The result is a web YouTube auth path that is harder to reason about, harder to recover from, and less robust than the reference flow migrated into `backend`.
...
- Clerk browser session availability via [web/clerk_bridge.js](app/web/clerk_bridge.js:297) and [lib/auth/clerk_web_bridge_web.dart](app/lib/auth/clerk_web_bridge_web.dart:86)
- OAuth initiation and callback handlers in [api/auth/youtube.js](app/api/auth/youtube.js:1), [api/auth/youtube/callback.js](app/api/auth/youtube/callback.js:1), and [api/auth/_youtube.js](app/api/auth/_youtube.js:1)
- Flutter bootstrap and Convex auth wiring in [lib/main.dart](app/lib/main.dart:1)
- YouTube connect UI and post-return feedback in [lib/widgets/youtube_connect.dart](app/lib/widgets/youtube_connect.dart:1)
...
- [lib/widgets/youtube_connect.dart](app/lib/widgets/youtube_connect.dart:1) is the main orchestration point and will lose popup-specific concerns while keeping route cleanup, feedback banner behavior, diagnostics, and refresh triggers.
- [api/auth/youtube.js](app/api/auth/youtube.js:1) and [api/auth/youtube/callback.js](app/api/auth/youtube/callback.js:1) will become easier to reason about by using redirect as the only supported web completion path.
- [lib/widgets/youtube_oauth_popup_bridge_web.dart](app/lib/widgets/youtube_oauth_popup_bridge_web.dart:1), [lib/widgets/youtube_oauth_popup_bridge.dart](app/lib/widgets/youtube_oauth_popup_bridge.dart:1), [lib/widgets/youtube_oauth_popup_bridge_stub.dart](app/lib/widgets/youtube_oauth_popup_bridge_stub.dart:1), and [lib/widgets/youtube_oauth_popup_result.dart](app/lib/widgets/youtube_oauth_popup_result.dart:1) become dead weight and should either be removed or left unused temporarily with a follow-up cleanup explicitly avoided in the same implementation if removal creates unnecessary churn.
...
- The app shell continues to mount [YoutubeOAuthFeedbackBanner](app/lib/widgets/youtube_connect.dart:481), so redirect-return state must stay compatible with the current shell-level post-auth banner approach.
