...
- Update `app/AGENT.md` if `Lists` vs YouTube Playlist vs ReplayGlowz Feed becomes an app-level contract.
- Update `backend/AGENT.md` or backend docs if new Convex tables/functions become formal integration boundaries.
- Update `shipflow_data/technical/app/architecture.md` and backend architecture docs if present.
