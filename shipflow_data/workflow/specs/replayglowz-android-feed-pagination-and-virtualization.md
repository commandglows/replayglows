...
supersedes: []
evidence:
  - "403-sf-perf audit on 2026-06-12 found `replayglowz_app/android/app/build.gradle.kts` shipping release builds without `isMinifyEnabled` or `isShrinkResources`."
  - "403-sf-perf audit on 2026-06-12 found `replayglowz_app/lib/screens/videos/videos_screen.dart` requesting `virtualFeedDetailsProvider(... pageSize: 500)` for every selected feed before merge."
  - "403-sf-perf audit on 2026-06-12 found `replayglowz_app/lib/widgets/media/media_thumbnail.dart` decoding remote thumbnails without render-size cache bounds; a first bounded fix is already in place."
...
La spec autorise une petite extension backend dans `replayglowz_backend` si le contrat Convex actuel ne permet pas un chargement progressif fiable. Cette extension reste dans le meme chantier car elle sert directement la promesse utilisateur et remplace une faiblesse structurelle deja observee.

## Scope In

- `replayglowz_app/android/app/build.gradle.kts` release shrink contract
- `replayglowz_app/android/app/proguard-rules.pro` keep rules minimales si une preuve le justifie
- `replayglowz_app/lib/screens/videos/videos_screen.dart`
- `replayglowz_app/lib/screens/playlists/virtual_feed_detail_screen.dart` et toute autre surface feed longue touchee par le meme pattern
- `replayglowz_app/lib/providers/providers.dart` pour le contrat `virtualFeedDetailsProvider` et ses args
...
- `automated_commands`:
  ...
  - `(cd replayglowz_app && flutter test <targeted feed/pagination tests>)`
  - `(cd replayglowz_app && flutter build apk --release)` or `(cd replayglowz_app && flutter build appbundle --release)` for shrink proof
  - `(cd replayglowz_backend/packages/backend && npm run typecheck)` only if Convex feed query shape changes
...
- `replayglowz_app/lib/widgets/media/media_thumbnail.dart`
- `replayglowz_app/android/app/build.gradle.kts`
- `replayglowz_app/android/app/proguard-rules.pro`
- `replayglowz_app/lib/main.dart`
- `replayglowz_app/lib/app/build_info.dart`
- `replayglowz_backend/packages/backend/convex/*` only if feed cursor/page contract changes
...
- [ ] Task 1: Lock the Android release shrinking contract and verify plugin safety.
  - File: `replayglowz_app/android/app/build.gradle.kts`, `replayglowz_app/android/app/proguard-rules.pro`
...
- [ ] Task 2: Define a bounded feed-window contract for filtered videos.
  - File: `replayglowz_app/lib/providers/providers.dart` and backend feed query files if needed
...
- [ ] Task 3: Rework `VideosScreen` filtered-feed loading to be progressive.
  - File: `replayglowz_app/lib/screens/videos/videos_screen.dart`
...
- [ ] Task 5: Prove memory and image behavior under long scrolling.
  - File: `replayglowz_app/lib/widgets/media/media_thumbnail.dart` and any list/card widgets that override image behavior
...
- Read first: `replayglowz_app/lib/screens/videos/videos_screen.dart`, `replayglowz_app/lib/providers/providers.dart`, `replayglowz_app/lib/screens/playlists/virtual_feed_detail_screen.dart`, `replayglowz_app/android/app/build.gradle.kts`, `replayglowz_app/lib/main.dart`.
