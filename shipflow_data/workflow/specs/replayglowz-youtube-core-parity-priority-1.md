...
evidence:
  ...
  - "Current Flutter feed has unimplemented local filter actions in `replayglowz_app/lib/screens/videos/videos_screen.dart`."
  - "Current Flutter app now routes sync through quota-safe primitives referenced in `replayglowz_app/AGENT.md` and backend `youtube:startQuotaSafeSync`."
...
- `replayglowz_app/AGENT.md`: update only if action contracts or runtime flow change beyond the existing quota-safe sync statement.
...
- `replayglowz_app/lib/i18n/en.dart` and `fr.dart`: keep visible user copy natural in each language; French copy must use accents.
...
- App: `replayglowz_app` Flutter web, Riverpod, GoRouter, Clerk JS bridge, Convex web bridge.
- Backend: `replayglowz_backend/packages/backend/convex` with YouTube cache/actions, quota metrics, hidden items, watched/progress, playlist order, video order, notes, comments, likes, channel links.
- Existing app contracts:
  - `replayglowz_app/AGENT.md` states Flutter must not loop over every playlist directly and must use `youtube:startQuotaSafeSync`.
...
- [ ] Task 1: Freeze the P1 action map and current route contracts.
  - Files: `replayglowz_app/lib/app/router.dart`, `replayglowz_app/lib/providers/providers.dart`, `replayglowz_app/lib/providers/mutations.dart`, `replayglowz_backend/packages/backend/convex/youtube.ts`.
...
- [ ] Task 3: Add video action menu parity on feed cards and rows.
  ...
  - Validation: `cd replayglowz_app && flutter analyze`; manual action QA from card and list modes.
...
- [ ] Task 4: Add reusable add-to-playlist modal for existing videos.
  - Files: new or existing widgets under `replayglowz_app/lib/widgets/media/` or `replayglowz_app/lib/screens/playlists/`, `replayglowz_app/lib/providers/providers.dart`, `replayglowz_app/lib/providers/mutations.dart`.
  ...
  - Validation: `cd replayglowz_app && flutter analyze`; `cd replayglowz_backend/packages/backend && npm run typecheck` if backend contracts change.
...
- [ ] Task 5: Complete playlist detail actions.
  - Files: `replayglowz_app/lib/screens/playlists/playlist_detail_screen.dart`, `replayglowz_app/lib/providers/mutations.dart`, `replayglowz_backend/packages/backend/convex/youtube.ts`.
  ...
  - Validation: `cd replayglowz_app && flutter analyze`; backend typecheck if action signatures change; manual playlist detail QA.
...
- [ ] Task 6: Harden playlist overview actions.
  - Files: `replayglowz_app/lib/screens/playlists/playlists_screen.dart`, `replayglowz_app/lib/widgets/media/playlist_card.dart`.
  ...
  - Depends on: Task 1.
  - Validation: `cd replayglowz_app && flutter analyze`; manual overview QA.
...
- [ ] Task 7: Bridge YouTube iframe state into Flutter web.
  - Files: `replayglowz_app/lib/widgets/play/web_youtube_embed.dart`, `replayglowz_app/lib/widgets/play/web_youtube_embed_web.dart`, `replayglowz_app/lib/widgets/play/player_panel.dart`, `replayglowz_app/lib/screens/play/play_screen.dart`.
  ...
  - Validation: `cd replayglowz_app && flutter analyze`; browser QA on desktop and mobile viewport.
...
- [ ] Task 8: Restore core play queue controls.
  - Files: `replayglowz_app/lib/screens/play/play_screen.dart`, `replayglowz_app/lib/widgets/play/playback_controls.dart`, `replayglowz_app/lib/widgets/play/player_panel.dart`.
  ...
  - Depends on: Task 7.
  - Validation: `cd replayglowz_app && flutter analyze`; manual queue QA from feed and playlist detail.
...
- [x] Task 9: Make quota state first-class on P1 actions.
  - Files: `replayglowz_app/lib/widgets/app_shell.dart`, `replayglowz_app/lib/screens/videos/videos_screen.dart`, `replayglowz_app/lib/screens/playlists/playlists_screen.dart`, `replayglowz_app/lib/screens/playlists/playlist_detail_screen.dart`, `replayglowz_app/lib/screens/stats/stats_screen.dart`.
  ...
  - Validation: `cd replayglowz_app && flutter analyze`; manual quota blocked/warned QA.
...
- [x] Task 10: Fill i18n strings for P1 surfaces.
  - Files: `replayglowz_app/lib/i18n/en.dart`, `replayglowz_app/lib/i18n/fr.dart`.
  ...
  - Validation: `cd replayglowz_app && flutter analyze`; source review for newly added visible strings.
...
- [x] Task 11: Add focused tests and manual QA scripts.
  - Files: Flutter tests where present or new focused tests under `replayglowz_app/test`, backend tests where present, plus QA notes in the spec or implementation report.
  ...
  - Validation: `cd replayglowz_app && flutter analyze`; `cd replayglowz_backend/packages/backend && npm run typecheck`; focused tests if feasible.
...
- Source checks:
  - `rg -n "open search delegate|show filter bottom sheet|direct fetchPlaylistItems|youtubeAccessToken|searchYoutubeVideos|search.list" replayglowz_app/lib replayglowz_backend/packages/backend/convex`
...
- Read first:
  ...
  - `replayglowz_app/lib/widgets/play/web_youtube_embed_web.dart`
...
- Update `replayglowz_app/AGENT.md` only if the action contracts or runtime flow change.
