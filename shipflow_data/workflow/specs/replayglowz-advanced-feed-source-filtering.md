...
- Main Feed filter UI in `replayglowz_app/lib/screens/videos/videos_screen.dart`.
...
- surface: Flutter web/mobile shared app code in `replayglowz_app`, Riverpod providers, local preference state, and read-only Convex virtual feed detail data.
...
- checklist_path: `shipflow_data/workflow/test-checklists/replayglowz-advanced-feed-source-filtering.md` must be created during implementation if widget tests cannot fully prove mobile interaction and Play queue behavior.
...
- `replayglowz_app/lib/screens/videos/videos_screen.dart` for the Feed view, filter picker, selected feed detail reads, merge logic, and Play queue source.
- `replayglowz_app/lib/models/virtual_feed.dart` for `VirtualFeed`, `VirtualFeedSource`, and `VirtualFeedDetails`.
- `replayglowz_app/lib/models/video.dart` for `feedSourceType`, `feedSourceId`, and `feedSourceTitle`.
- `replayglowz_app/lib/providers/providers.dart` for `virtualFeedDetailsProvider`.
- `replayglowz_app/lib/i18n/en.dart` and `replayglowz_app/lib/i18n/fr.dart` for user-facing copy.
- Existing specs:
  - `shipflow_data/workflow/specs/replayglowz-virtual-feeds-channel-aggregators.md`
...
- Update `replayglowz_app/CHANGELOG.md` when implemented.
- Update `shipflow_data/technical/apps/replayglowz_app/context.md` and/or `architecture.md` only if the implementation introduces a durable new local preference contract or helper abstraction.
...
- [ ] Task 2: Extract source/video filtering helpers.
  - File: `replayglowz_app/lib/screens/videos/videos_screen.dart` or a small app-local helper under `replayglowz_app/lib/screens/videos/`.
...
- [ ] Task 6: Add docs/checklist updates if implementation needs them.
  - Files: `replayglowz_app/CHANGELOG.md`, optionally `shipflow_data/workflow/test-checklists/replayglowz-advanced-feed-source-filtering.md`, optionally app technical docs.
  ...
  - Validate with: metadata lint if ShipFlow docs are changed.
...
- Run Flutter Web/mobile preview smoke after ship:
  ...
  - clear filters.
- Run metadata lint if ShipFlow docs/checklists are added:
...
- Read first:
  ...
  - `replayglowz_app/lib/i18n/en.dart` and `fr.dart`
