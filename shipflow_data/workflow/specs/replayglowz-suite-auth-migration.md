...
| sf-auth-debug | fixing | Production browser proof identified a CSP boot blocker: CanvasKit from `www.gstatic.com` is denied before Flutter renders, and Clerk's live ReplayGlowz frontend API domain is absent from the auth directives. Local Vercel config is patched. | sf-ship |
| sf-verify | pending | Hosted user-flow proof remains after the CSP fix deploys: app boot, Clerk sign-in/session restore, entitlement denied/allowed behavior, and YouTube OAuth start/callback must be exercised with a real signed-in session. | sf-prod |
