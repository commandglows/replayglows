...
| sf-auth-debug | fixing | Production browser proof identified and fixed the first CSP boot blocker. The latest deployed browser proof now shows the Flutter/Clerk scripts load, but no first frame is painted; production console logging is being added to expose the Dart-side blocker. | sf-ship |
