...
evidence:
  ...
  - "ReplayGlowz currently uses Firebase Auth as the web session owner in `replayglowz_app/lib/auth/auth_service.dart`."
...
- `/home/claude/shipflow_data/projects/winflowz/docs/technical/suite-authentication.md`: decision canonique Clerk central + Firebase Android bridge + entitlements serveur.
- `/home/claude/shipflow_data/projects/winflowz/docs/technical/suite-authentication-support-runbook.md`: support et triage des entitlements, duplicate email, bridge failure et wrong environment.
- `/home/claude/winflowz_app/shipflow_data/workflow/specs/unified-suite-authentication.md`: chantier actif de l'identite suite.
...
- `replayglowz_app/lib/main.dart`: bootstrap Firebase + Convex token wiring actuel.
- `replayglowz_app/lib/convex/convex_client.dart`: injection token Convex et bridge HTTP web.
- `replayglowz_app/api/auth/youtube.js`: demarrage OAuth YouTube avec Firebase ID token.
- `replayglowz_app/api/auth/youtube/callback.js`: callback YouTube qui reutilise le Firebase ID token comme JWT Convex.
- `replayglowz_app/web/index.html`: chargement des scripts web bridge.
- `replayglowz_app/web/clerk_bridge.js`: nouveau bridge ClerkJS cible a creer.
- `replayglowz_app/lib/auth/clerk_js_bridge.dart`: nouveau wrapper Dart cible a creer.
- `replayglowz_app/tool/check_shared_backend_contract.dart`: preflight du backend produit via `REPLAYGLOWZ_BACKEND_ROOT`.
- `replayglowz_app/build.sh`, `.env.example`, `vercel.json`: env et CSP deployes.
...
- `replayglowz_app/README.md`: setup, env vars, auth model and validation commands.
...
- [ ] Tache 3 : Add the ClerkJS bridge for Flutter web
  - Fichier : `replayglowz_app/web/index.html`, `replayglowz_app/web/clerk_bridge.js`, `replayglowz_app/lib/auth/clerk_js_bridge.dart`
  ...
  - Depends on : Tache 2.
  - Validate with : `cd replayglowz_app && flutter analyze` et browser check de `/sign-in`.
...
- [ ] Tache 4 : Replace Firebase web session owner with suite auth service
  - Fichier : `replayglowz_app/lib/auth/auth_service.dart`, `replayglowz_app/lib/auth/auth_state.dart`
  ...
  - Depends on : Tache 3.
  - Validate with : `cd replayglowz_app && flutter analyze`, tests unitaires auth service avec fake bridge.
  - Notes : Garder `auth_state.dart` SDK-neutral; aucune fusion email cote client.

- [ ] Tache 5 : Adapt sign-in UI and auth diagnostics
  - Fichier : `replayglowz_app/lib/auth/auth_gate.dart`
...
- [ ] Tache 6 : Rewire bootstrap and ReplayGlowz product Convex token provider
  - Fichier : `replayglowz_app/lib/main.dart`, `replayglowz_app/lib/convex/convex_client.dart`
...
- [ ] Tache 9 : Add product entitlement gate in client-visible flow
  - Fichier : `replayglowz_app/lib/providers/providers.dart` ou provider cible equivalent.
...
- [ ] Tache 10 : Replace Firebase token handoff in YouTube OAuth start
  - Fichier : `replayglowz_app/lib/widgets/youtube_connect.dart`
...
- [ ] Tache 11 : Replace Firebase token cookie in YouTube OAuth API
  - Fichier : `replayglowz_app/api/auth/youtube.js`
  ...
  - Validate with : `cd replayglowz_app && node --test api/auth/_youtube.test.js`.
...
- [ ] Tache 12 : Replace Firebase token use in YouTube OAuth callback
  - Fichier : `replayglowz_app/api/auth/youtube/callback.js`
...
- [ ] Tache 13 : Update CSP and Vercel deployment config for Clerk/suite auth
  - Fichier : `replayglowz_app/vercel.json`
...
- [ ] Tache 14 : Update docs and operator diagnostics
  - Fichier : `replayglowz_app/README.md`, `replayglowz_app/AGENT.md`, `replayglowz_app/CLAUDE.md`, `replayglowz_app/CHANGELOG.md`
...
- [ ] Tache 15 : Hosted auth verification
  - Fichier : deployment Vercel `replayglowz_app`
  ...
  - Validate with : `/sf-prod replayglowz_app`, `/sf-auth-debug https://app.replayglowz.com ReplayGlowz suite auth`, puis `/sf-verify replayglowz-suite-auth-migration`.
...
- Prod proof: `/sf-prod replayglowz_app` before browser/user-flow conclusions.
...
| sf-start | implemented | ReplayGlowz app codepath, product backend, suite verifier endpoint, server-to-server entitlement route contract, and required Production env names are now in place. Google OAuth config failure is resolved after redeploy; unauthenticated OAuth start now fails closed with missing suite session token. | sf-auth-debug |
| sf-auth-debug | fixing | Production browser proof identified and fixed the first CSP boot blocker, then traced the remaining blank frame through deployed console logs. Local patch now switches Flutter web to path URLs and lets the app paint before bootstrap auth/Convex wiring completes. | sf-ship |
| sf-verify | pending | Hosted user-flow proof remains after the blank first-frame blocker is fixed: app boot, Clerk sign-in/session restore, entitlement denied/allowed behavior, and YouTube OAuth start/callback must be exercised with a real signed-in session. | sf-prod |
