...
| sf-verify | pending | Hosted user-flow proof remains after operator browser retest: app boot, Clerk sign-in/session restore, entitlement denied/allowed behavior, and YouTube OAuth start/callback must be exercised with a real signed-in session. | sf-test |
