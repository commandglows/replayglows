...
| 2026-06-12 13:40:00 UTC | sf-migrate | GPT-5 Codex + Codex Spark explorer | Researched the official `firebase-admin` 14.0.0 breaking changes, created backup branch `migrate/firebase-admin-14-backup`, mapped local FCM usage, and attempted the major upgrade. | Blocked: the codebase is already on modular imports, but `npm install` rejects `firebase-admin@14.0.0` under `min-release-age=7` because the release was published on 2026-06-08. | Retry `/404-sf-migrate ReplayGlowz backend firebase-admin 14 upgrade` after 2026-06-15 UTC or make an explicit supply-chain policy decision. |
