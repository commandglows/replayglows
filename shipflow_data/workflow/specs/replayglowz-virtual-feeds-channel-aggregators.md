...
owner: "Diane"
user_story: "En tant qu'utilisateur ReplayGlowz connecte a YouTube, je veux creer des feeds virtuels qui regroupent les chaines et sources qui m'interessent, afin de lancer une lecture thematique sans depenser du quota en ajoutant chaque video dans une playlist YouTube."
confidence: "high"
risk_level: "high"
security_impact: "yes"
docs_impact: "yes"
...
next_step: "/sf-start replayglowz-virtual-feeds-channel-aggregators"
---

# Spec: ReplayGlowz Virtual Feeds Channel Aggregators

## Title

ReplayGlowz virtual feeds channel aggregators
...
En tant qu'utilisateur ReplayGlowz connecte a YouTube, je veux creer des feeds virtuels qui regroupent les chaines et sources qui m'interessent, afin de lancer une lecture thematique sans depenser du quota en ajoutant chaque video dans une playlist YouTube.
...
- Preconditions: l'utilisateur est authentifie via Clerk, dispose d'un acces ReplayGlowz actif, Convex auth est pret, et ses abonnements/playlists/videos YouTube sont deja caches ou synchronisables via les flux quota-safe existants.
...
- i18n anglais/francais pour noms de surfaces, etats vides, erreurs, actions locales, et distinction Feed ReplayGlowz vs playlist YouTube.

## Scope Out

- Pas de creation, modification ou suppression automatique de playlists YouTube depuis un Feed ReplayGlowz.
...
- Les Feeds sont des ressources ReplayGlowz, pas des ressources YouTube; leur CRUD doit rester sans appel YouTube externe.
...
- ReplayGlowz cache-first behavior remains: showing cached videos must not trigger YouTube API calls.
...
- Product consequence: `Lists` becomes the umbrella surface for YouTube playlists and ReplayGlowz Feeds; public copy must not imply virtual Feeds edit YouTube playlists.
...
- Update `replayglowz_app/AGENT.md` if `Lists` vs YouTube Playlist vs ReplayGlowz Feed becomes an app-level contract.
- Update `replayglowz_backend/AGENT.md` or backend docs if new Convex tables/functions become formal integration boundaries.
- Update `shipflow_data/technical/apps/replayglowz_app/architecture.md` and backend architecture docs if present.
...
- [ ] Task 1: Design backend schema for virtual Feeds.
  - File: `replayglowz_backend/packages/backend/convex/schema.ts`
  ...
  - User story link: Gives ReplayGlowz a local organization layer independent from YouTube playlists.
  - Depends on: None
...
- [ ] Task 3: Implement cached aggregation query.
  - File: `replayglowz_backend/packages/backend/convex/virtualFeeds.ts`, possibly helper in `youtube.ts`
...
- [ ] Task 5: Rename Playlists surface to Lists and add Feed cards.
  - File: `replayglowz_app/lib/screens/playlists/`, `replayglowz_app/lib/app/router.dart`, `replayglowz_app/lib/widgets/app_shell.dart`, `replayglowz_app/lib/i18n/en.dart`, `replayglowz_app/lib/i18n/fr.dart`
  - Action: Rename user-facing Playlists to `Lists`, keep legacy route redirects, and show both YouTube playlist cards and ReplayGlowz Feed cards with distinct icons/labels/actions.
...
- [ ] Task 6: Add source picker from subscriptions and playlists.
  - File: `replayglowz_app/lib/screens/playlists/`, `replayglowz_app/lib/screens/preferences/preferences_screen.dart`
...
- [ ] Task 8: Add i18n, copy and quota-safe affordances.
  - File: `replayglowz_app/lib/i18n/en.dart`, `replayglowz_app/lib/i18n/fr.dart`, `replayglowz_app/lib/widgets/youtube_quota_guard.dart`
...
- [ ] CA 1: Given a connected user with cached subscriptions, when they open `Lists`, then they see YouTube playlists and ReplayGlowz Feeds as distinct list types.
...
- surface: ReplayGlowz Flutter app `Lists`, Feed detail, Play queue, Convex virtual feed functions, and quota guard.
...
- checklist_path: `shipflow_data/workflow/audits/replayglowz-virtual-feeds-channel-aggregators-qa.md` if manual QA is recorded as a durable checklist during implementation.
...
- Manual QA:
  - Open `Lists` and confirm YouTube playlists and ReplayGlowz Feeds are distinguishable.
...
- Product naming confusion: mitigate with explicit `ReplayGlowz Feed` copy and by keeping YouTube Playlists separate.
...
|----------|-------|-------|--------|--------|-----------|
| 2026-05-28 17:52:05 UTC | sf-spec | GPT-5 Codex | Created draft spec for ReplayGlowz virtual Feeds after product decision to avoid costly YouTube playlist writes for channel grouping. | draft spec created | `/sf-ready replayglowz-virtual-feeds-channel-aggregators` |
| 2026-05-28 18:08:00 UTC | sf-ready | GPT-5 Codex | Integrated operator decision that Feeds live inside Playlists renamed to Lists, added Test Contract, reviewed structure, scope, security, quota and acceptance criteria. | ready | `/sf-start replayglowz-virtual-feeds-channel-aggregators` |
| 2026-05-28 22:29:00 UTC | sf-start | GPT-5.3 Codex Spark delegated sequential | Implemented backend virtual feed schema/functions, Dart model/provider/mutation baseline, Lists cards, create flow, and feed detail route; source management, complete playback return behavior, i18n split, and source-scan tests remain. | partial | `/sf-start replayglowz-virtual-feeds-channel-aggregators continue source-picker-playback-i18n` |
| 2026-05-28 22:29:00 UTC | sf-verify | GPT-5 Codex | Verified local checks, quota-safety source scan for virtual feed surfaces, and acceptance coverage; implementation is not complete enough for clean verification. | partial | `/sf-start replayglowz-virtual-feeds-channel-aggregators continue source-picker-playback-i18n` |
| 2026-05-29 03:21:50 UTC | sf-start | GPT-5.3 Codex (degraded fallback; requested GPT-5.3 Codex Spark at capacity) | Completed source picker/add-remove-toggle-reorder UX with cache-empty/error + quota-aware refresh path, finalized virtual Feed play flow/queue handoff + return-to-active scroll behavior, and finished EN/FR Lists vs ReplayGlowz Feed copy updates with local validation and quota-safety scans. | implemented | `/sf-verify replayglowz-virtual-feeds-channel-aggregators` |
| 2026-05-29 03:41:48 UTC | sf-verify | GPT-5 Codex | Re-ran backend typecheck, Flutter analyze/tests, ShipFlow metadata lint, and targeted quota-safety scan; implementation satisfies local contract, with hosted preview/browser/manual proof still pending because development mode is vercel-preview-push. | partial | `sf-ship -> sf-prod -> sf-test --preview` |
...
| sf-verify | partial | Local verification passes: backend typecheck, Flutter analyze/tests, metadata lint, targeted quota-safety scan, ownership/source validation review. Hosted preview/browser/manual QA remains pending under vercel-preview-push mode. |
| sf-end | pending | Close trackers/docs after verification. |
| sf-ship | pending | Ship only bounded spec files/implementation changes. |
