...
depends_on:
  ...
  - artifact: "shipflow_data/workflow/specs/replayglowz-app-performance-loading-data.md"
    artifact_version: "1.0.0"
    required_status: "reviewed"
...
evidence:
  ...
  - "Current Flutter helper `replayglowz_app/lib/providers/mutations.dart` calls `youtube:fetchYoutubePlaylists` and then batches `youtube:fetchPlaylistItems` for every playlist."
  - "Current backend `replayglowz_backend/packages/backend/convex/metrics.ts` defines YouTube quota costs and plan limits but the sync UI does not use a central quota gate before triggering full sync."
...
- Update `replayglowz_app/AGENT.md` if the app contract changes from client-driven sync to backend-orchestrated sync.
- Update `replayglowz_backend/AGENT.md` or backend docs if sync job tables/actions become part of the public backend contract.
...
- [ ] Task 1: Audit and freeze the current YouTube sync call graph.
  - File: `replayglowz_app/lib/providers/mutations.dart`, `replayglowz_app/lib/screens/videos/videos_screen.dart`, `replayglowz_app/lib/screens/playlists/playlists_screen.dart`, `replayglowz_app/lib/widgets/youtube_connect*.dart`, `replayglowz_backend/packages/backend/convex/youtube.ts`
  ...
  - Depends on: None
  - Validate with: source review and `rg -n "syncAllPlaylists|fetchYoutubePlaylists|fetchPlaylistItems" replayglowz_app replayglowz_backend -S`
  - Notes: Do not change OAuth start/callback behavior in this task.

- [ ] Task 2: Add backend quota planning helpers.
  - File: `replayglowz_backend/packages/backend/convex/metrics.ts`, `replayglowz_backend/packages/backend/convex/youtube.ts`
...
- [ ] Task 3: Add sync job state and per-user lock.
  - File: `replayglowz_backend/packages/backend/convex/schema.ts`, `replayglowz_backend/packages/backend/convex/youtube.ts`
...
- [ ] Task 4: Replace client-driven full sync with backend-orchestrated bounded sync.
  - File: `replayglowz_backend/packages/backend/convex/youtube.ts`, `replayglowz_app/lib/providers/mutations.dart`
...
- [ ] Task 6: Centralize Flutter quota and sync progress state.
  - File: `replayglowz_app/lib/providers/providers.dart`, new focused provider file if needed, `replayglowz_app/lib/screens/stats/stats_screen.dart`
...
- [ ] Task 7: Add visible quota indicator and sync progress UX.
  - File: `replayglowz_app/lib/widgets/app_shell.dart`, `replayglowz_app/lib/screens/videos/videos_screen.dart`, `replayglowz_app/lib/screens/playlists/playlists_screen.dart`, `replayglowz_app/lib/widgets/youtube_connect_ui_states.dart`, `replayglowz_app/lib/i18n/en.dart`, `replayglowz_app/lib/i18n/fr.dart`
...
- [ ] Task 8: Enforce cache-first screen behavior.
  - File: `replayglowz_app/lib/screens/videos/videos_screen.dart`, `replayglowz_app/lib/screens/playlists/playlists_screen.dart`, `replayglowz_app/lib/providers/providers.dart`
...
- [ ] Task 9: Add tests and diagnostics.
  - File: `replayglowz_backend/packages/backend/convex/*`, `replayglowz_app/test/**`, existing diagnostics widgets
  ...
  - Validate with: `cd replayglowz_backend/packages/backend && npm run typecheck`, `cd replayglowz_app && flutter test`, `cd replayglowz_app && flutter analyze`
...
- [ ] Task 10: Document and deploy safely.
  - File: `replayglowz_app/CHANGELOG.md`, backend/app AGENT docs if contracts changed, deployment env docs
...
- Read first: `replayglowz_app/lib/providers/mutations.dart`, `replayglowz_app/lib/providers/providers.dart`, `replayglowz_app/lib/screens/videos/videos_screen.dart`, `replayglowz_app/lib/screens/playlists/playlists_screen.dart`, `replayglowz_app/lib/screens/stats/stats_screen.dart`, `replayglowz_backend/packages/backend/convex/youtube.ts`, `replayglowz_backend/packages/backend/convex/metrics.ts`, `replayglowz_backend/packages/backend/convex/schema.ts`.
