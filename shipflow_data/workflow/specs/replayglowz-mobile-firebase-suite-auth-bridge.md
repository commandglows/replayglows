...
| 2026-06-10 20:52:23 UTC | sf-fix | GPT-5 Codex | Investigated user retest diagnostics from run `27302777396`, fixed native Firebase init order by deferring `FirebaseAuth.instance` until after `FirebaseBootstrap.initialise()`, pinned `flutter_rust_bridge` to `2.11.1` for `convex_flutter 3.0.1`, and added a constructor regression test. | fix-attempted: local `flutter analyze` and `flutter test` pass; lockfile now resolves `flutter_rust_bridge 2.11.1`. Remaining proof is a new GitHub Actions Android APK artifact and device retest. | Run new Android CI build and retest uploaded GitHub artifact on device. |
