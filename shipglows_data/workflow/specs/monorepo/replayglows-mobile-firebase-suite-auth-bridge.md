...
depends_on:
  ...
  - artifact: "/home/claude/winflowz_app/shipglows_data/workflow/specs/firebase-backend-agnostic-migration.md"
    artifact_version: "1.0.0"
    required_status: "ready"
...
- `cd app && flutter test`
- `cd app && dart run tool/check_shared_backend_contract.dart`
- `cd app && node --test api/auth/_youtube.test.js`
...
- `app/lib/auth/auth_service.dart`: current ClerkJS web owner to split by platform.
- `app/lib/auth/clerk_js_bridge_stub.dart`: current non-web unsupported path.
- `app/lib/convex/convex_client.dart`: non-web Convex token refresh hook.
- `app/lib/providers/providers.dart`: `productAccessStatusProvider` and private provider gating surface.
- `app/lib/widgets/youtube_connect.dart`: YouTube OAuth start gate.
- `app/api/auth/youtube.js` and `api/auth/youtube/callback.js`: OAuth server path that must accept only verified product sessions.
...
- `app/README.md`: native Firebase auth setup, bridge URL, Android CI secrets, and web ClerkJS preservation.
- `app/AGENT.md`: add platform-specific auth owner and native fail-closed bridge rules.
- `app/CLAUDE.md`: add Firebase native risk areas and validation commands.
...
- [ ] Tache 1 : Introduce platform-specific auth adapter contract
  - Fichiers : `app/lib/auth/auth_service.dart`, `app/lib/auth/auth_state.dart`, new `app/lib/auth/auth_session_adapter.dart`
...
- [ ] Tache 2 : Add Firebase native bootstrap and dependencies
  - Fichiers : `app/pubspec.yaml`, `app/lib/core/bootstrap/firebase_bootstrap.dart`, `app/android/**`, `app/.env.example`
...
- [ ] Tache 3 : Implement native Firebase auth adapter
  - Fichiers : `app/lib/auth/firebase_auth_adapter.dart`, `app/lib/auth/auth_service.dart`, tests under `app/test/`
...
- [ ] Tache 7 : Gate product access server-side in product Convex
  - Fichiers : `backend/packages/backend/convex/users.ts`, shared auth/access helper modules, private function modules as needed
...
- [ ] Tache 8 : Wire native Convex auth and provider selection
  - Fichiers : `app/lib/main.dart`, `app/lib/convex/convex_client.dart`, `app/lib/providers/providers.dart`
...
- [ ] Tache 9 : Preserve and test web ClerkJS path
  - Fichiers : `app/lib/auth/clerk_js_bridge*.dart`, `web/clerk_bridge.js`, `lib/main.dart`, `vercel.json`
...
- [ ] Tache 10 : Add native YouTube OAuth start gating
  - Fichiers : `app/lib/widgets/youtube_connect.dart`, `app/api/auth/youtube.js`, product backend ticket/session helper
...
- [ ] Tache 11 : Complete native YouTube OAuth callback handling
  - Fichiers : `app/api/auth/youtube/callback.js`, `app/android/app/src/main/AndroidManifest.xml`, Flutter route/deep-link handling if selected
...
- [ ] Tache 13 : Update docs and support diagnostics
  - Fichiers : `app/README.md`, `AGENT.md`, `CLAUDE.md`, `.env.example`, `CHANGELOG.md`, technical architecture docs
