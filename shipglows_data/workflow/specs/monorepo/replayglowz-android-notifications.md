...
supersedes: []
evidence:
  - "app/lib/screens/preferences/preferences_screen.dart already exposes notification preferences."
  - "app/lib/screens/notifications/notifications_screen.dart already displays in-app notifications and mark-as-read actions."
  - "app/lib/models/notification.dart supports `new_video`, `transcript_ready`, and `system` notification types."
  - "backend/packages/backend/convex/notifications.ts already stores and serves notification records."
  - "backend/packages/backend/convex/feedChecker.ts already creates `new_video` notifications from backend checks."
...
- `app/lib/screens/notifications/notifications_screen.dart`
...
- `backend/packages/backend/convex/feedChecker.ts`
...
- `backend` gains a new device-registration model and push-delivery action layer.
...
- Update `app/README.md` with Android notification setup and any required Firebase plugin/config steps.
...
- [x] Task 3: Add server-side push delivery adapter using Firebase Admin via Convex actions.
  - Files:
    ...
    - `backend/packages/backend/package.json` if dependencies are added
...
- [x] Task 5: Add Android app messaging integration and permission flow.
  - Files:
    - `app/pubspec.yaml`
    - Android platform files under `app/android/`
    - new messaging/bootstrap files under `app/lib/`
...
- [x] Task 9: Document setup, secrets, and QA procedure.
  - Files:
    ...
    - optional checklist file under `shipglows_data/workflow/test-checklists/`
