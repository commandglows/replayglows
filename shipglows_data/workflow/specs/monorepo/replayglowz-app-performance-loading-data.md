...
- Flutter web app code under `app/`.
...
- [x] CA 7: `flutter analyze` passes in `app`.
- [x] CA 8: `flutter build web --release` passes in `app`.
...
- [x] Task 1: Inspect route graph and heavy imports.
  - Files: `app/lib/app/router.dart`, play screen files, feedback screen files, feedback admin files.
...
- [x] Task 2: Defer or isolate heavy route code paths where feasible.
  - Files: `app/lib/app/router.dart` and any small route-loader wrappers created under `app/lib/`.
...
- [x] Task 3: Gate PlayScreen transcript loading by active tab.
  - Files: `app/lib/screens/play/play_screen.dart` and related provider helpers only if needed.
...
- [x] Task 4: Remove all-videos dependency from PlayScreen when safe.
  - Files: `app/lib/screens/play/play_screen.dart`, route arguments/helpers, and existing app model/provider files only if needed.
...
- [x] Task 5: Review AppShell connection banner provider usage.
  - Files: `app/lib/widgets/app_shell.dart` and existing YouTube connection providers/widgets.
...
- Flutter app static check: `cd app && flutter analyze`.
- Flutter web build: `cd app && flutter build web --release`.
- Local size observations after build:
  - `du -sh app/build/web`
  - inspect `app/build/web/main.dart.js`
  - inspect `app/build/web/canvaskit/canvaskit.wasm` and `app/build/web/canvaskit/skwasm.wasm` when present.
