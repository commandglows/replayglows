...
evidence:
  ...
  - "No dedicated Priority 2 YouTube feature parity spec existed in `shipglows_data/workflow/specs` before this run."
...
- Local contracts:
  - `app/AGENT.md` runtime flow and YouTube boundary.
...
- Update `app/AGENT.md` only if the runtime contract changes beyond exposing existing backend primitives.
...
- [x] Task 1: Freeze P2 backend action/query contracts.
  - File: `backend/packages/backend/convex/channelLinks.ts`, `backend/packages/backend/convex/transcripts.ts`, `backend/packages/backend/convex/transcriptGeneration.ts`, `backend/packages/backend/convex/transcriptSecrets.ts`, `backend/packages/backend/convex/notes.ts`, `app/lib/providers/providers.dart`, `app/lib/providers/mutations.dart`.
  ...
  - Validate with: `rg -n "channelLinks|transcriptSecrets|transcriptGeneration|selectTranscriptVersion|exportNotesForVideo|subscription" app/lib backend/packages/backend/convex`.
...
- [x] Task 3: Build linked-channel management UI.
  - File: `app/lib/screens/preferences/preferences_screen.dart`, `app/lib/screens/playlists/playlist_detail_screen.dart`, new focused widget under `app/lib/widgets/` if needed.
...
- [x] Task 4: Harden channel sync backend error/status behavior if needed.
  - File: `backend/packages/backend/convex/channelLinks.ts`, `backend/packages/backend/convex/youtube.ts`, `backend/packages/backend/convex/metrics.ts`.
...
- [x] Task 5: Add Flutter transcript provider catalog and secret status providers.
  - File: `app/lib/models/settings.dart`, `app/lib/models/`, `app/lib/providers/providers.dart`, `app/lib/providers/mutations.dart`.
...
- [x] Task 6: Build transcript provider management UI.
  - File: `app/lib/screens/preferences/preferences_screen.dart`, `app/lib/widgets/settings/settings_rows.dart`, new transcript settings widgets if needed.
...
- [x] Task 7: Complete transcript versions and job UX in player.
  - File: `app/lib/screens/play/play_screen.dart`, `app/lib/widgets/transcripts/transcript_entry_tile.dart`, providers from Task 5.
...
- [x] Task 9: Add P2 onboarding and persistent UX helpers.
  - File: `app/lib/widgets/youtube_connect_ui_states.dart`, `app/lib/screens/videos/videos_screen.dart`, `app/lib/screens/playlists/playlists_screen.dart`, `app/lib/screens/preferences/preferences_screen.dart`, existing settings/preferences model if persistence belongs backend.
...
- [x] Task 10: Add focused tests, diagnostics and documentation updates.
  ...
  - Validate with: `(cd app && flutter analyze)`, `(cd backend/packages/backend && npm run typecheck)`, `/home/claude/shipglows/tools/shipglows_metadata_lint.py AGENTS.md shipglows_data`.
...
- [ ] Static checks pass: Flutter analyze, backend typecheck if backend touched, and ShipGlows metadata lint.
...
- Source checks:
  - `rg -n "search\\.list|searchYoutube|youtubeAccessToken|refreshToken|clientSecret" app/lib backend/packages/backend/convex`
  - `rg -n "channelLinks|transcriptSecrets|exportNotesForVideo|selectTranscriptVersion" app/lib backend/packages/backend/convex`
