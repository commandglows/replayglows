...
linked_systems:
  ...
  - "YouTube Data API"
depends_on:
  - "shipglows_data/workflow/specs/replayglows-youtube-channel-onboarding-playlist-url-import.md"
...
# ReplayGlows Prod QA Followups - 2026-05-26

## Scope Covered

This QA pass covers the three recently implemented YouTube parity specs:

- `replayglows-youtube-core-parity-priority-2.md`: channel automation/subscriptions, transcript provider management, transcript jobs/versions, notes export/share/copy, P2 onboarding.
- `replayglows-youtube-core-parity-priority-3.md`: UX settings, dismissible hints, persisted view preferences, scroll restoration, player focus/shortcuts, i18n.
- `replayglows-youtube-channel-onboarding-playlist-url-import.md`: YouTube channel onboarding and playlist URL import.
...
- Test account class: ReplayGlows account authenticated with YouTube connected, no YouTube subscriptions, imported playlist `Fun`.
...
| URL import | Imported playlist preserved in Playlists | pass | Playlist `Fun`, `1 video`, updated May 26, 2026. | `test-evidence/replayglows-prod-three-spec-qa/playlists-imported-state.png` |
| URL import | Imported video visible in Cards/List/Notes views | pass | Cards, List and Notes tab all show the imported video. | QA screenshots in `/tmp/replayglows-browser-check/qa-0*.png` |
...
| URL import | Automatic sync preserves imported URL playlists | partial | Playlist stayed visible during navigation; not proven after a deliberate automatic sync with empty `mine=true`. | `test-evidence/replayglows-prod-three-spec-qa/playlists-imported-state.png` |
| P2 | YouTube connected state in Preferences | pass | Preferences shows connected state and reconnect/sync/disconnect controls. | `test-evidence/replayglows-prod-three-spec-qa/preferences-transcript-settings.png` |
| P2 | No subscriptions empty state | pass | Preferences shows "No YouTube subscriptions found" and explains this is normal for a new account. | `test-evidence/replayglows-prod-three-spec-qa/preferences-provider-channel-empty.png` |
| P2 | Transcript provider catalog | pass-visible | Provider list renders: YouTube captions available, other providers unavailable in environment. | `test-evidence/replayglows-prod-three-spec-qa/preferences-provider-channel-empty.png` |
...
| P3 | Feed Cards/List/Notes views | pass | All three render imported video. | `/tmp/replayglows-browser-check/qa-01-videos-cards.png`, `/tmp/replayglows-browser-check/qa-02-videos-list.png`, `/tmp/replayglows-browser-check/qa-04-videos-notes-tab.png` |
| P3 | View preference persistence | pass-basic | List tab remained selected after reload. | `/tmp/replayglows-browser-check/qa-03-videos-list-after-reload.png` |
| P3 | Notes empty state | pass | Notes page shows helpful empty state and search. | `test-evidence/replayglows-prod-three-spec-qa/notes-empty-state.png` |
| P3 | Player and YouTube embed | pass | Imported video opens in Now Playing with YouTube embed. | `/tmp/replayglows-browser-check/qa-08-play-screen.png` |
| P3 | Keyboard shortcuts helper | pass | Shortcut dialog opens from hint button and top keyboard icon. | `test-evidence/replayglows-prod-three-spec-qa/player-shortcuts-dialog.png` |
| P3 | Hint dismissal persistence | not run | Hints were observed, but close/reload persistence was not tested. | none |
| P3 | Focus/study panel behavior | partial | Player UI and shortcuts render; panel toggles/focus persistence not fully verified. | `/tmp/replayglows-browser-check/qa4-08-player-panels.png` |
| P3 | Route/deep-link persistence | pass | Production smoke on 2026-05-27 verified signed-out redirect to `/sign-in?tf_redirect=/playlists` and authenticated direct `/preferences`, `/playlists`, `/notes`, and `/videos` route retention/rendering when browser locale was explicit. | `shipglows_data/workflow/bugs/app/BUG-2026-05-26-001.md` |
| P3 | Playlists low-friction add UI | pass | Authenticated production retest on 2026-05-27 confirmed the low-opacity `+` is visible on the short imported-playlist page and opens the in-place `New Playlist` modal. The modal was canceled; no playlist was created. | `shipglows_data/workflow/bugs/app/BUG-2026-05-26-002.md` |
| Quota | Cache-first navigation | pass-smoke-pending-metrics | Production smoke on 2026-05-27 kept visible quota at `13 / 1000` across Preferences, Playlists, Notes, and Videos without sync/import/refresh. Convex log/metrics binding remains pending. | `shipglows_data/workflow/bugs/app/BUG-2026-05-26-003.md` |
...
|----|----------|--------|---------|----------|
| BUG-2026-05-26-001 | medium | fixed-prod-verify | Direct protected routes and signed-out redirect passed production smoke. | `shipglows_data/workflow/bugs/app/BUG-2026-05-26-001.md` |
| BUG-2026-05-26-002 | medium | closed | Short/non-scrollable Playlists page `+` is reachable and opens the create modal in authenticated production QA. | `shipglows_data/workflow/bugs/app/BUG-2026-05-26-002.md` |
| BUG-2026-05-26-003 | medium | mitigated-partial-prod-verify | Hidden quota spend path identified in code (`fetchYoutubeSubscriptions` on passive Preferences load), moved behind explicit refresh, and smoke-tested visually with stable quota. Metrics-bound proof remains pending. | `shipglows_data/workflow/bugs/app/BUG-2026-05-26-003.md` |
