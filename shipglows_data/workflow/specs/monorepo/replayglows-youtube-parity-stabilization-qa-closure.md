...
depends_on:
  ...
  - artifact: "shipglows_data/workflow/bugs/app/BUG-2026-05-26-001.md"
    artifact_version: "0.1.0"
    required_status: "draft"
  - artifact: "shipglows_data/workflow/bugs/app/BUG-2026-05-26-002.md"
    artifact_version: "0.1.0"
    required_status: "draft"
  - artifact: "shipglows_data/workflow/bugs/app/BUG-2026-05-26-003.md"
    artifact_version: "0.1.0"
    required_status: "draft"
...
- La copie visible par l'utilisateur doit etre naturelle dans la langue active de l'interface; les nouvelles strings francaises doivent etre accentuees dans `fr.dart`, tandis que les headings/cles internes ShipGlows restent en anglais.
...
- Local files likely touched:
  ...
  - `app/lib/screens/playlists/create_playlist_screen.dart`
  ...
  - `app/lib/widgets/ui_hint_card.dart`
  - `app/lib/widgets/youtube_channel_onboarding.dart`
  ...
  - `backend/packages/backend/convex/transcripts.ts`
  - `backend/packages/backend/convex/transcriptGeneration.ts`
  - `backend/packages/backend/convex/transcriptSecrets.ts`
  - `backend/packages/backend/convex/notes.ts`
...
- [ ] Task 2: Fix protected route and deep-link semantics.
  - File: `app/lib/app/router.dart`, `app/lib/widgets/app_shell.dart`, `app/lib/auth/auth_gate.dart`, `app/lib/widgets/youtube_connect.dart`.
...
- [ ] Task 3: Fix Playlists onboarding and create/edit UX.
  - File: `app/lib/screens/playlists/playlists_screen.dart`, `app/lib/screens/playlists/create_playlist_screen.dart`, `app/lib/screens/playlists/playlist_detail_screen.dart`, `app/lib/widgets/youtube_channel_onboarding.dart`, `app/lib/i18n/en.dart`, `app/lib/i18n/fr.dart`.
...
- [ ] Task 4: Reproduce and close the hidden quota-spend question.
  - File: `backend/packages/backend/convex/youtube.ts`, `backend/packages/backend/convex/metrics.ts`, `app/lib/providers/providers.dart`, `app/lib/widgets/app_shell.dart`, `shipglows_data/workflow/bugs/app/BUG-2026-05-26-003.md`.
...
- [ ] Task 5: Harden and retest playlist URL import edge cases.
  - File: `backend/packages/backend/convex/youtube.ts`, `app/lib/providers/mutations.dart`, `app/lib/widgets/youtube_channel_onboarding.dart`, `app/lib/screens/playlists/playlists_screen.dart`, `app/lib/i18n/en.dart`, `app/lib/i18n/fr.dart`.
...
- [ ] Task 6: Complete P2 transcript provider and transcript job QA.
  - File: `app/lib/screens/preferences/preferences_screen.dart`, `app/lib/screens/play/play_screen.dart`, `app/lib/providers/providers.dart`, `app/lib/providers/mutations.dart`, `backend/packages/backend/convex/transcripts.ts`, `backend/packages/backend/convex/transcriptGeneration.ts`, `backend/packages/backend/convex/transcriptSecrets.ts`.
...
- [ ] Task 7: Complete P2 notes export/share/copy QA.
  - File: `app/lib/screens/notes/notes_screen.dart`, `app/lib/screens/notes/note_detail_screen.dart`, `app/lib/screens/play/play_screen.dart`, `backend/packages/backend/convex/notes.ts`, `app/lib/providers/mutations.dart`.
...
- [ ] Task 8: Complete P2 channel link/sync QA with a suitable account or mark deferred.
  - File: `app/lib/screens/preferences/preferences_screen.dart`, `app/lib/screens/playlists/playlist_detail_screen.dart`, `backend/packages/backend/convex/channelLinks.ts`, `backend/packages/backend/convex/youtube.ts`.
...
- [ ] Task 9: Complete P3 UX persistence, focus, shortcuts and mobile QA.
  - File: `app/lib/widgets/ui_hint_card.dart`, `app/lib/screens/videos/videos_screen.dart`, `app/lib/screens/playlists/playlists_screen.dart`, `app/lib/screens/play/play_screen.dart`, `app/lib/screens/notes/notes_screen.dart`, `app/lib/models/settings.dart`.
...
- [ ] Task 10: Add focused automated coverage for fixed regressions.
  - File: `app/test/`, `backend/packages/backend/convex/`, existing test locations if present.
...
- [ ] Task 11: Run focused checks and deployment verification.
  - File: changed subprojects plus `shipglows_data/workflow/*`.
  - Action: Run `(cd app && flutter analyze)`, `(cd backend/packages/backend && npm run typecheck)` if backend changed, `/home/claude/shipglows/tools/shipglows_metadata_lint.py AGENT.md shipglows_data`, then preview/prod browser QA according to deployment mode.
...
- [ ] Required checks pass for touched subprojects and ShipGlows metadata lint passes.
...
- Static checks:
  ...
  - `(cd backend/packages/backend && npm run typecheck)` when backend changed.
...
| 2026-05-26 19:55:00 UTC | sf-prod | GPT-5 Codex | Confirmed Vercel production deployment `dpl_9DM2pzMHBonJfrWg7uZpuodDgtF7` for commit `39c6062`, aliases including `https://app.replayglows.com`, and HTTP 200 for root and `/playlists`. | pass | `/sf-verify replayglows-youtube-parity-stabilization-qa-closure` |
...
| 2026-05-27 09:04:31 UTC | sf-ship | GPT-5 Codex | Prepared quick ship for the bounded Playlists short-page `+` fix and associated bug/audit/test/spec trace updates. | shipped | `/sf-prod app` then targeted `/sf-verify`. |
| 2026-05-27 10:47:12 UTC | sf-browser | GPT-5 Codex | Checked `https://app.replayglows.com/playlists` in Chromium after deploy: no-locale headless reproduces the known locale bootstrap crash/fallback; with `--lang=en-US`, build `7f97ab2` loads and the protected route reaches the Sign in screen. | needs-auth | Use `sf-auth-debug` or an authenticated Playwright storage state to verify the protected Playlists `+` modal. |
...
| sf-prod | done | Production deployment `dpl_9DM2pzMHBonJfrWg7uZpuodDgtF7` is ready and aliased to `https://app.replayglows.com`; root and `/playlists` returned HTTP 200. |
