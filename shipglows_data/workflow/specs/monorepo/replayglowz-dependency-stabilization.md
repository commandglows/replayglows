...
- [x] Task 7: Switch worker Docker install to locked dependencies
  ...
  - Depends on: Task 6.
  - Validate with: `cd lab && docker build -t replayglowz-transcript-worker-deps-test .` when Docker is available; otherwise run Python import and compile checks locally.
...
- Python/worker:
  ...
  - `cd lab && docker build -t replayglowz-transcript-worker-deps-test .` when Docker is available
- Flutter/app:
  - `cd app && flutter pub get`
  - `cd app && flutter pub outdated --json`
  - `cd app && flutter analyze`
- Governance/docs:
  - `/home/claude/shipglows/tools/shipglows_metadata_lint.py AGENT.md shipglows_data shipglows_data/workflow/specs/replayglowz-dependency-stabilization.md`
  - `git diff --check`
...
- Write ownership: `.github/dependabot.yml`, root `README.md`, `shipglows_data/workflow/specs/replayglowz-dependency-stabilization.md`, and final integration notes only.
...
| Step | Status | Notes |
|------|--------|-------|
| sf-spec | done | Draft spec created in `shipglows_data/workflow/specs/replayglowz-dependency-stabilization.md`. |
