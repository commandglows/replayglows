...
evidence:
  ...
  - "Audit found `app/lib/screens/play/play_screen.dart` is 1037 lines and mixes player, playback controls, notes, transcript, comments placeholder, queue, options, mutations, and error handling."
  - "Audit found `app/lib/widgets/youtube_connect.dart` is 1267 lines and mixes OAuth flow helpers, connection banners, settings cards, diagnostics, and empty states."
...
- Refactor of screen rendering code in:
  ...
  - `app/lib/screens/hidden/hidden_screen.dart`
- Theme/token cleanup inside `app/lib/app/theme.dart` only when needed to support extracted components.
- Narrow runtime theme selector wiring in `app/lib/app/theme.dart`, `app/lib/main.dart`, and existing settings provider/model code paths, limited to making persisted `settings.theme` affect `MaterialApp.themeMode`.
- Focused widget tests under `app/test/**` for newly extracted component contracts and the theme-mode mapping/bridge. The test folder does not exist yet and may be created.
...
- Update `app/README.md` or `app/AGENT.md` only if the implementation establishes a durable component organization convention that future agents should follow.
...
- [ ] Task 1: Establish app UI primitive folder and token helpers.
  - File: `app/lib/widgets/app_states.dart` or `app/lib/widgets/app_states/*.dart`
...
- [ ] Task 2: Add media display components for videos and playlists.
  - File: `app/lib/widgets/media/video_card.dart`, `app/lib/widgets/media/video_list_tile.dart`, `app/lib/widgets/media/playlist_card.dart`, or equivalent paths.
...
- [ ] Task 3: Add note and transcript display components.
  - File: `app/lib/widgets/notes/note_tile.dart`, `app/lib/widgets/notes/note_group_header.dart`, `app/lib/widgets/transcripts/transcript_entry_tile.dart`, or equivalent paths.
...
- [ ] Task 4: Split YouTube connection UI from OAuth/helper logic.
  - File: `app/lib/widgets/youtube_connect.dart` plus new files such as `youtube_connect_flow.dart`, `youtube_connect_banner.dart`, `youtube_connection_card.dart`, `youtube_empty_states.dart`.
...
- [ ] Task 5: Refactor Videos, Playlists, Hidden, and Notes screens to use shared components.
  - File: `app/lib/screens/videos/videos_screen.dart`, `app/lib/screens/playlists/playlists_screen.dart`, `app/lib/screens/hidden/hidden_screen.dart`, `app/lib/screens/notes/notes_screen.dart`.
...
- [ ] Task 6: Split PlayScreen into panel components.
  - File: `app/lib/screens/play/play_screen.dart` plus new files under `app/lib/screens/play/widgets/` or `app/lib/widgets/play/`.
...
- [ ] Task 7: Refactor PreferencesScreen settings rows.
  - File: `app/lib/screens/preferences/preferences_screen.dart` plus new widgets such as `settings_section.dart`, `settings_choice_tile.dart`, `settings_switch_tile.dart`.
...
- [ ] Task 8: Wire the runtime theme selector and perform focused token cleanup only where needed.
  - File: `app/lib/app/theme.dart`
  - File: `app/lib/main.dart`
  - File: `app/lib/providers/providers.dart`
  - File: `app/lib/models/settings.dart`
...
- [ ] Task 9: Add focused widget tests for component contracts.
  - File: `app/test/widgets/**` or equivalent `app/test/**` paths created during implementation.
...
- [ ] CA 2: `flutter analyze` passes in `app`.
...
- [ ] CA 12: Focused widget tests are added under `app/test/**` and `flutter test` passes.

## Test Strategy

- Static validation: `cd app && flutter analyze`.
- Widget tests: create focused tests under `app/test/**`, then run `cd app && flutter test`.
- Root governance validation after spec or docs updates: `/home/claude/shipglows/tools/shipglows_metadata_lint.py AGENT.md shipglows_data`.
- Manual QA checklist:
...
- Medium test-infra risk: the app currently has no `app/test` directory, so the first tests must stay small and fake-driven to avoid brittle auth/network setup.
...
- Read these files first before implementation: `app/lib/app/theme.dart`, `app/lib/main.dart`, `app/lib/providers/providers.dart`, `app/lib/models/settings.dart`, `app/lib/screens/preferences/preferences_screen.dart`, `app/lib/widgets/youtube_connect.dart`, `app/lib/screens/play/play_screen.dart`, and one representative library screen before extracting shared widgets.
...
- Validate incrementally with `cd app && flutter analyze`; after tests are introduced, also run `cd app && flutter test`.
