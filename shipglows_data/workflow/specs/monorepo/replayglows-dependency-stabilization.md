...
updated: "2026-05-28"
updated_at: "2026-05-28 14:16:07 UTC"
status: active
source_skill: sf-spec
source_model: "GPT-5 Codex"
scope: "audit-fix"
owner: "Diane"
user_story: "As the ReplayGlows maintainer, I want the Flutter app, Astro site, and transcript worker dependencies patched, pinned, audited, and automatically monitored so security fixes and deploys are reproducible without weakening runtime behavior."
confidence: "high"
risk_level: "medium"
security_impact: "yes"
docs_impact: "yes"
...
evidence:
  ...
  - "site/npm audit --json reports GHSA-j687-52p2-xcff for astro <6.1.6 and GHSA-qx2v-qp2m-jg93 for postcss <8.5.10."
  ...
  - "lab/pip-audit -r requirements.txt --no-deps --disable-pip -f json reports CVE-2026-25645 / GHSA-gc5v-m9x4-r6x2 for requests==2.32.5, fixed in 2.33.0."
  - "Requests vendor advisory: https://github.com/psf/requests/security/advisories/GHSA-gc5v-m9x4-r6x2"
  - "app/flutter pub outdated --json reports no current pub advisories but multiple patch/minor/major upgrade lanes."
...
next_step: "/sf-ship ReplayGlows dependency stabilization --bounded-scope"
---

# Title

ReplayGlows Dependency Stabilization
...
As the ReplayGlows maintainer, I want the Flutter app, Astro site, and transcript worker dependencies patched, pinned, audited, and automatically monitored so security fixes and deploys are reproducible without weakening runtime behavior.
...
- Starting from the current audit state, `site` upgrades within the Astro 6 and PostCSS 8 lines so `npm audit --json` no longer reports GHSA-j687-52p2-xcff or GHSA-qx2v-qp2m-jg93.
- `site/package-lock.json` remains the committed npm lockfile, `npm run build` succeeds, and public SEO/i18n content behavior remains unchanged.
- `lab` upgrades `requests` to at least `2.33.0` and uses a deterministic install path for the worker, preferably `requirements.in` plus a generated hash-checked lock file installed by Docker and documented for operators.
- `lab` keeps the `/transcribe` response contract: `entries`, `fullText`, `estimatedCostUsd`, `warnings`.
- `app` preserves its current app behavior while adding a repeatable dependency audit/update path and pinning the Flutter toolchain used by deployment and Android CI.
- `.github/dependabot.yml` covers npm for `site`, pip or pip-compile for `lab`, pub for `app`, GitHub Actions, and Docker base image updates, with review-friendly cadence and no silent automerge.
...
- Severite: P2
- Scope: `site`, `app`, `lab`, CI/deploy dependency policy
- Spec recommandee: `/sf-spec ReplayGlows dependency stabilization: patch Astro/PostCSS and requests advisories, add update automation, pin package/tool versions, and add reproducible Python locking/auditing`
...
- Patch `site` Astro/PostCSS advisories by updating `package.json` and `package-lock.json` within compatible major ranges.
- Patch `lab` Requests advisory by updating the worker dependency to `requests>=2.33.0,<3`.
...
- Pin or document the Node/npm package manager used by `site`.
...
- Preserve the repository guidance in `AGENT.md`: subproject contracts are source evidence, Astro runtime content frontmatter must not receive ShipGlows metadata, and unrelated dirty files must not be touched.
- Preserve `app` boundaries: backend schema/functions stay out of this repo, Convex transport remains in `lib/convex/`, and build-time values stay `String.fromEnvironment(...)` backed.
- Preserve `site` SEO behavior: canonical URLs, `hreflang`, Open Graph tags, JSON-LD, and content collection schema must remain valid.
- Preserve `lab` worker contract: `POST /transcribe` returns `entries`, `fullText`, `estimatedCostUsd`, and `warnings`; `youtube_captions` remains out of worker scope.
...
- `site`: npm, Astro 6, Tailwind CSS 4, `package-lock.json`.
- `lab`: FastAPI, Uvicorn, Requests, yt-dlp, faster-whisper, OpenAI Python SDK, FunASR, Docker.
- `app`: Flutter, Dart pub, Clerk beta packages, Convex, Riverpod, GoRouter.
...
- `site/package.json` and `site/package-lock.json`: patch Astro/PostCSS and add package manager metadata.
- `site/README.md` and `site/CLAUDE.md`: align install commands to `npm ci` when lockfile-based installs are expected, plus audit/update commands.
- `lab/requirements.txt`: either becomes generated lock output or remains direct pins only if a separate lock file is introduced and Docker uses it.
- `lab/requirements.in`: direct worker dependencies source if pip-tools is adopted.
- `lab/requirements.lock` or generated `requirements.txt`: transitive hash-checked lock consumed by Docker and CI.
- `lab/Dockerfile`: install from the locked/hash-checked file and keep `ffmpeg` install behavior.
- `lab/README.md`: update local, Docker, and PM2 setup commands for the lock/audit strategy.
- `app/pubspec.yaml` and `app/pubspec.lock`: remove unused deps only after verification, and keep direct/runtime deps correctly placed.
- `app/vercel.json`: replace `git clone -b stable` with a pinned Flutter SDK strategy or an explicit documented stable version source.
- `app/.github/workflows/android-apk.yml`: pin Flutter action input beyond floating `stable` when feasible.
- `.github/dependabot.yml`: new root automation config with multiple directory entries.
- Root `README.md`, `AGENT.md`, `shipglows_data/workflow/TASKS.md`: docs only if implementation changes command expectations; `sf-spec` itself does not update tasks.
...
- Update `site/README.md` if the preferred install command becomes `npm ci` and if dependency scanning commands are added.
- Update `lab/README.md` wherever it currently says `pip install -r requirements.txt`; it must name the authoritative locked install file and how to regenerate it.
- Update `lab/AGENT.md` only if the source-of-truth dependency file changes from `requirements.txt` to `requirements.in` plus lock.
- Update `app/README.md`, `app/AGENT.md`, or `app/CLAUDE.md` only if Flutter SDK pinning changes local/deploy commands.
...
- [x] Task 1: Add dependency automation scaffold
  ...
  - Action: Create Dependabot v2 config covering `npm` in `/site`, `pip` in `/lab`, `pub` in `/app`, `github-actions` in `/`, and `docker` in `/lab`; use weekly or grouped schedules and no automerge.
...
- [x] Task 2: Patch Astro/PostCSS advisories within current majors
  ...
  - Validate with: `cd site && npm install && npm audit --json && npm run build`.
...
- [x] Task 3: Commit the site lockfile result
  ...
  - Depends on: Task 2.
  - Validate with: `cd site && npm ci && npm audit --json && npm run build`.
...
- [x] Task 4: Add Node/npm package-manager pin for the site
  ...
  - Depends on: Task 3.
  - Validate with: `cd site && npm ci && npm run build`.
...
- [x] Task 5: Separate Python direct requirements from generated lock input
  ...
  - Depends on: None.
  - Validate with: `cd lab && python3 -m pip install pip-tools && pip-compile --generate-hashes --output-file requirements.lock requirements.in`.
...
- [x] Task 6: Generate and use Python hash lock
  ...
  - Depends on: Task 5.
  - Validate with: `cd lab && python3 -m pip install --require-hashes -r requirements.lock` in a disposable environment.
...
- [x] Task 7: Switch worker Docker install to locked dependencies
  ...
  - Depends on: Task 6.
  - Validate with: `cd lab && docker build -t replayglows-transcript-worker-deps-test .` when Docker is available; otherwise run Python import and compile checks locally.
...
- [x] Task 8: Update worker docs for lock and audit workflow
  ...
  - Validate with: `rg -n "requirements\\.(txt|in|lock)|pip-audit|pip-compile" lab/README.md`.
...
- [x] Task 9: Add or update worker agent guidance if source files change
  ...
  - Validate with: `/home/claude/shipglows/tools/shipglows_metadata_lint.py lab/AGENT.md`.
...
- [x] Task 10: Pin Flutter SDK/deploy toolchain
  ...
  - Depends on: None.
  - Validate with: `cd app && flutter --version && flutter pub get && flutter analyze`.
...
- [x] Task 11: Pin Android CI Flutter version
  - File: `app/.github/workflows/android-apk.yml`
  ...
  - Depends on: Task 10.
  - Validate with: `cd app && flutter pub get && flutter analyze`; workflow validation by GitHub Actions on branch if available.
...
- [x] Task 12: Verify and clean likely unused Flutter dependencies
  ...
  - Validate with: `cd app && flutter pub get && flutter analyze`.
...
- [x] Task 13: Update Flutter dependency docs only where commands change
  ...
  - Validate with: `rg -n "Flutter|flutter pub|get|outdated|analyze" app/README.md app/AGENT.md`.
...
- [x] Task 15: Final verification pass
  ...
  - Validate with: `cd site && npm audit --json && npm run build`; `cd lab && pip-audit -r requirements.lock --require-hashes -f json && python -m py_compile main.py server.py`; `cd app && flutter pub outdated --json && flutter analyze`; `/home/claude/shipglows/tools/shipglows_metadata_lint.py AGENT.md shipglows_data shipglows_data/workflow/specs/replayglows-dependency-stabilization.md`.
  - Notes: If Docker is available, include `docker build` for the worker; if not, list Docker as unverified.

# Acceptance Criteria

- [x] CA 1: Given the current `site` audit findings, when compatible npm updates are applied, then `npm audit --json` no longer reports GHSA-j687-52p2-xcff or GHSA-qx2v-qp2m-jg93.
- [x] CA 2: Given the updated site lockfile, when `npm ci` and `npm run build` run in `site`, then the production Astro build succeeds.
...
- `npm ci`, `npm audit --json`, and `npm run build` passed in `site`; audit metadata reports zero vulnerabilities.
- `lab` lock resolves `requests==2.33.1`, `fastapi==0.124.4`, and `starlette==0.50.0`; fallback audit of the disposable locked install reported no known vulnerabilities.
...
- `python -m py_compile main.py server.py` passed for `lab`.
- `flutter pub get`, `flutter pub outdated --json`, and `flutter analyze` passed in `app`; `flutter pub outdated` reported no current advisory or retraction flags.
...
- `site/CHANGELOG.md`, `lab/CHANGELOG.md`, and `app/CHANGELOG.md` record the dependency, Docker, and CI changes.
- ShipGlows metadata lint passed for root governance docs, the spec, and changed worker docs.
...
- Node/site:
  - `cd site && npm ci`
  - `cd site && npm audit --json`
  - `cd site && npm run build`
- Python/worker:
  - `cd lab && pip-compile --generate-hashes --output-file requirements.lock requirements.in`
  - `cd lab && python -m pip install --require-hashes -r requirements.lock` in a disposable environment
  - `cd lab && pip-audit -r requirements.lock --require-hashes -f json`
  - `cd lab && python -m py_compile main.py server.py`
  - `cd lab && docker build -t replayglows-transcript-worker-deps-test .` when Docker is available
- Flutter/app:
  - `cd app && flutter pub get`
  - `cd app && flutter pub outdated --json`
  - `cd app && flutter analyze`
- Governance/docs:
  - `/home/claude/shipglows/tools/shipglows_metadata_lint.py AGENT.md shipglows_data shipglows_data/workflow/specs/replayglows-dependency-stabilization.md`
  - `git diff --check`
...
- Read first:
  ...
  - `lab/Dockerfile`
  - `app/vercel.json`
  - `app/.github/workflows/android-apk.yml`
  - `app/pubspec.yaml`
...
- Packages allowed:
  - Use npm for `site` because `package-lock.json` is committed.
...
- Packages or moves to avoid:
  - Do not introduce pnpm/yarn to `site`.
...
- Write ownership: `site/package.json`, `site/package-lock.json`, `site/README.md`, and site-local dependency notes only.
- Forbidden files: `app/**`, `lab/**`, root `README.md`, `.github/**`, `shipglows_data/workflow/bugs/app/**`, and `specs/**`.
...
- Per-batch validation: `(cd site && npm ci)`, `(cd site && npm audit --json)`, `(cd site && npm run build)`, and `git diff --check -- site`.
- Integration owner: `sf-build` integrator.

## Batch B: Worker Python Locking

- Write ownership: `lab/requirements.txt`, `lab/requirements.in`, `lab/requirements.lock`, `lab/Dockerfile`, `lab/README.md`, and `lab/AGENT.md`.
- Forbidden files: `app/**`, `site/**`, root `README.md`, `.github/**`, `shipglows_data/workflow/bugs/app/**`, and `specs/**`.
- Tasks covered: Tasks 5, 6, 7, 8, and 9.
- Dependency order: Can run after readiness; independent from A and C.
- Per-batch validation: `(cd lab && python -m py_compile main.py server.py)`, a disposable locked install when feasible, `pip-audit -r requirements.lock -f json` or documented audit fallback, optional Docker build when Docker is available, and `git diff --check -- lab`.
- Integration owner: `sf-build` integrator.

## Batch C: App Toolchain And Dependency Hygiene

- Write ownership: `app/vercel.json`, `app/.github/workflows/android-apk.yml`, `app/pubspec.yaml`, `app/pubspec.lock`, `app/README.md`, and app-local dependency notes only.
- Forbidden files: `app/lib/auth/auth_gate.dart`, other unrelated dirty app source files, `site/**`, `lab/**`, root `README.md`, root `.github/**`, `shipglows_data/workflow/bugs/app/**`, and `specs/**`.
- Tasks covered: Tasks 10, 11, 12, and 13.
- Dependency order: Can run after readiness; independent from A and B.
- Per-batch validation: import/usage search for candidate dependency removals, `(cd app && flutter pub get)`, `(cd app && flutter pub outdated --json)`, `(cd app && flutter analyze)`, and `git diff --check -- app`.
- Integration owner: `sf-build` integrator.

## Batch D: Automation, Shared Docs, And Final Integration

- Write ownership: `.github/dependabot.yml`, root `README.md`, `shipglows_data/workflow/specs/replayglows-dependency-stabilization.md`, and final integration notes only.
- Forbidden files: `shipglows_data/workflow/bugs/app/**`, `app/lib/auth/auth_gate.dart`, and any subproject files already owned by active Batch A, B, or C workers.
...
- Per-batch validation: YAML parse for `.github/dependabot.yml` when PyYAML is available, ShipGlows metadata lint, full targeted cross-project checks from Task 15, and `git diff --check`.
- Integration owner: `sf-build` integrator.
...
|----------|-------|-------|--------|--------|-----------|
| 2026-05-10 21:05:58 UTC | sf-spec | GPT-5 Codex | Created dependency stabilization chantier spec from sf-deps intake and local/external evidence. | Draft spec saved. | /sf-ready ReplayGlows dependency stabilization |
| 2026-05-10 21:16:26 UTC | sf-ready | GPT-5.5 medium subagent + GPT-5 Codex | Validated required sections, documentation freshness, security review, and added safe Execution Batches for spec-gated parallelism. | Ready. | /sf-start ReplayGlows dependency stabilization |
| 2026-05-10 21:39:16 UTC | sf-start | GPT-5.5 medium subagents + GPT-5 Codex | Implemented dependency patches, automation, toolchain pins, Python hash lock, changelogs, and docs through safe batches A-D. | Implemented with Docker proof gap. | /sf-verify ReplayGlows dependency stabilization |
| 2026-05-10 21:39:16 UTC | sf-verify | GPT-5 Codex | Ran site, worker, app, metadata, YAML, audit, and diff validations against the spec. | Partial: local checks pass; Docker image build unavailable. | /sf-ship ReplayGlows dependency stabilization --bounded-scope |
...
| 2026-05-11 13:57:33 UTC | sf-deps | GPT-5 Codex | Re-audited `app` Pub graph with Flutter 3.41.7 / Dart 3.11.5, OSV batch query, license pass, import usage checks, and `flutter analyze`. | B-: no advisories; medium follow-ups remain for Clerk beta patch, unused codegen deps, and Sentry/lints major lanes. | Decide whether to patch Clerk beta and remove unused codegen packages in a bounded app dependency pass. |
...
| 2026-05-13 08:47:57 UTC | sf-ship | GPT-5 Codex | Shipped stable Firebase Auth replacement for the beta-Clerk removal follow-up, Vercel Firebase envs, and Convex Firebase auth config. | Shipped with hosted auth/OAuth validation pending. | /sf-prod app |
| 2026-05-14 10:59:44 UTC | sf-prod | GPT-5 Codex | Checked Vercel production deployment for commit `09c3749`, live app assets, OAuth start handler, runtime logs, and Convex prod deploy readiness. | Partial: Vercel ready and app envs baked; Convex prod deploy dry-run still reports missing `FIREBASE_PROJECT_ID` for auth config. | Set/verify `FIREBASE_PROJECT_ID` on Convex prod deployment `joyous-chipmunk-990`, deploy Convex, then run `/sf-prod app` again. |
| 2026-05-28 14:16:07 UTC | sf-tasks | GPT-5 Codex | Migrated the project-local task tracker to canonical traffic-first task records and reconciled evidence-backed task status. | Tracker updated; chantier state unchanged. | Set/verify `FIREBASE_PROJECT_ID` on Convex prod deployment `joyous-chipmunk-990`, deploy Convex, then `/sf-prod app`. |
...
Next command: set/verify `FIREBASE_PROJECT_ID=winflowz-dev` on Convex prod deployment `joyous-chipmunk-990`, deploy Convex, then `/sf-prod app`
